...
if __name__ == "__main__":
    ...
    # 主页设置
    st.set_page_config(page_title="B 站集市低价搜索", page_icon=get_path("./bilibili-line.png"))

    # 侧栏设置
    with st.sidebar:
        st.header("🎁 B 站集市低价搜索")
        st.info("这里是介绍，但不知道写什么")
        ...
        with st.form(key='my_form'):
            ...
            st.write('🔗 [B站集市链接](https://mall.bilibili.com/neul-next/index.html?page=magic-market_index)')

            submitted = st.form_submit_button(label='搜索', type="primary", use_container_width=True, disabled=st.session_state['form_submit_button_visible'])
    ...
    if submitted:
        if cookie:
            ...
            with st.status("🌐 爬取集市数据中 ..."):
                ...
                while 1:
                    ...
                    for i in response.json()['data']['data']:
                        ...
                        item_data['discount'] = f"{(item_data['price'] / item_data['market_price'])*10:.1f} 折"
                        # item_data['img'] = 'https:'+i['detailDtoList'][0]['img']
                        item_data[
                            'url'] = "https://mall.bilibili.com/neul-next/index.html?page=magic-market_detail&noTitleBar=1&itemsId=" + str(
                            i['c2cItemsId']) + "&from=market_index"
            ...
            st.success(f"Crawling count: {len(result)}, Crawling time: {(end_time - start_time)/60:.2f} min")
            ...
            min_price_df = pd.merge(min_price_df, result_df['name'].value_counts(), on='name', how='outer')
            new_order = ['column1', 'column2', 'column3']
            min_price_df = min_price_df.reindex(columns=["name", "count", "price", "market_price", "discount", "url"])
            st.dataframe(min_price_df,
                         column_config={
